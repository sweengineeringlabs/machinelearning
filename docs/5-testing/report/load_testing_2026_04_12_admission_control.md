...
> **Client**: `llminference/main/features/inference/systemd/scripts/load_test.sh N` (bash + curl, N parallel subshells)
...
`llminference/main/features/inference/systemd/main/src/core/router.rs` calls `tokio::task::spawn_blocking` directly in both `handle_blocking` and `handle_streaming`. There is no semaphore, no queue, no per-request timeout governor. The tokio blocking pool has a 512-thread default ceiling, so 50 requests all get CPU immediately and compete for memory.
...
./llminference/main/features/inference/systemd/scripts/load_test.sh 16   # or any N
...
Output lands in `llminference/main/features/inference/systemd/scripts/out/` (gitignored): one `stat_$i.txt` and `resp_$i.json` per request.
