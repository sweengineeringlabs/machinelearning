...
`main/features/inference/daemon/scripts/load_test.sh` is a 50-line bash + curl burst driver. It proves "the server survives a synchronous burst" and nothing else — no percentiles, capped at ~20 concurrent by Cygwin fork limits, one workload, one endpoint.
...
```
main/features/inference/daemon/scripts/
...
load-smoke:
  needs: [build-release]
  steps:
    - run: ./main/features/inference/daemon/scripts/loadtest/run.sh baseline chat_short
