...
fn bench_rms_norm() {
    ...
    let dim = 1152;
    let x = swe_ml_tensor::Tensor::randn(vec![1, 1, dim]);
    let weight = swe_ml_tensor::Tensor::ones(vec![dim]);
    let eps = 1e-6f32;
...
fn bench_rope() {
    ...
    let rope = swe_llmmodel_layers::RoPEFreqs::new(head_dim, 1024, 10000.0);
...
fn bench_inplace_ops() {
    ...
    let a = swe_ml_tensor::Tensor::randn(vec![1, 1, dim]);
    let b = swe_ml_tensor::Tensor::randn(vec![1, 1, dim]);
    ...
    let mut x = swe_ml_tensor::Tensor::randn(vec![1, 1, dim]);
    // Warmup
    for _ in 0..1000 {
...
fn bench_softmax_rayon_threshold() {
    ...
    let small = swe_ml_tensor::Tensor::from_vec(small_data, vec![1, 32, 1, 64]).unwrap();
    ...
    let large = swe_ml_tensor::Tensor::from_vec(large_data, vec![1, 32, 1, 256]).unwrap();
...
fn bench_batched_matmul_rayon_threshold() {
    ...
    let q = swe_ml_tensor::Tensor::from_vec(q_data, vec![32, 1, 64]).unwrap();
    let kt = swe_ml_tensor::Tensor::from_vec(kt_data, vec![32, 64, 128]).unwrap();
    ...
    let attn = swe_ml_tensor::Tensor::from_vec(attn_data, vec![32, 1, 128]).unwrap();
    let v = swe_ml_tensor::Tensor::from_vec(v_data, vec![32, 128, 64]).unwrap();
...
fn bench_inplace_score_scaling() {
    ...
    let scores_template = swe_ml_tensor::Tensor::from_vec(scores_data, vec![1, 32, 1, 128]).unwrap();
