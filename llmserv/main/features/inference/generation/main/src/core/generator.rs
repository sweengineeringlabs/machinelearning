...
impl<'a> Generator<'a> {
    ...
    fn encode_prompt(&self, prompt: &str) -> GenerationResult<Vec<u32>> {
    ...
    fn prefill(&self, tokens: &[u32], cache: &mut KVCache) -> GenerationResult<Vec<f32>> {
    ...
    fn decode_step(&self, token: u32, cache: &mut KVCache) -> GenerationResult<Vec<f32>> {
    ...
    fn validate_params(&self) -> GenerationResult<()> {
        if self.temperature < 0.0 {
            return Err(crate::api::error::GenerationError::Generation(
    ...
    fn check_deadline(&self) -> GenerationResult<()> {
    ...
    pub fn generate(&self, prompt: &str, max_tokens: usize) -> GenerationResult<String> {
        ...
        for _ in 1..max_tokens {
            ...
            for beam in beams.into_iter() {
                ...
                let last_token = *beam.tokens.last().ok_or_else(|| {
                    crate::api::error::GenerationError::Generation(
    ...
    /// Messages are `(role, content)` pairs where role is `"user"` or `"assistant"`.
    pub fn encode_conversation(&self, messages: &[(&str, &str)]) -> GenerationResult<Vec<u32>> {
        let template = match self.chat_template {
            Some(ref tmpl) => tmpl,
            None => {
