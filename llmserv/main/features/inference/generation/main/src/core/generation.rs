...
impl<'a> TextGenerator<'a> {
    ...
    pub fn generate(&self, input_ids: &Tensor, config: &GenerationConfig) -> GenerationResult<Tensor> {
    ...
    fn top_k_filter(&self, probs: &Tensor, k: usize) -> GenerationResult<Tensor> {
    ...
    fn top_p_filter(&self, probs: &Tensor, p: f32) -> GenerationResult<Tensor> {
        let vocab_size = probs.shape()[0];
    ...
    fn sample_from_probs<R: Rng>(&self, probs: &Tensor, rng: &mut R) -> GenerationResult<usize> {
        let r: f32 = rng.r#gen();
        let mut cumulative = 0.0;
...
mod tests {
    use super::*;
    use rustml_model::GptConfig;
    use rustml_model::GptModel;
