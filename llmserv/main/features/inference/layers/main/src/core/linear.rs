...
impl Linear {
    ...
    pub fn quantize_weight_q8(&mut self) -> NnResult<()> {
        ...
        let q8_bytes = swe_llmmodel_kernel::quantize_q8_0(f32_data)?;
    ...
    pub fn quantize_weight_q4_0(&mut self) -> NnResult<()> {
        ...
        let f32_data = self.weight.data()?;
        let q4_bytes = swe_llmmodel_kernel::quantize_q4_0(f32_data)?;
    ...
    pub fn quantize_weight_q4_1(&mut self) -> NnResult<()> {
        ...
        let f32_data = self.weight.data()?;
        let q4_bytes = swe_llmmodel_kernel::quantize_q4_1(f32_data)?;
    ...
    ///
    /// For Q4_0 weights uses `swe_llmmodel_kernel::matmul_f32_q4` (or the native
    /// integer variant when `use_native_q4` is set).  Q8_0 weights use
    /// `swe_llmmodel_kernel::matmul_f32_q8`.  All other dtypes fall back to the
    ...
    pub fn forward(&self, x: &Tensor) -> NnResult<Tensor> {
        ...
        let output = if self.weight.dtype() == DType::Q4_0 {
            ...
            let result_data = if self.use_native_q4 {
                swe_llmmodel_kernel::matmul_f32_q4_native(x_data, w_bytes, m, in_features, out_features)?
            } else {
                swe_llmmodel_kernel::matmul_f32_q4(x_data, w_bytes, m, in_features, out_features)?
            };
            ...
            *out_shape.last_mut().unwrap() = out_features;
            ...
            let result_data = if self.use_native_q4 {
                swe_llmmodel_kernel::matmul_f32_q4_1_native(x_data, w_bytes, m, in_features, out_features)?
            } else {
                swe_llmmodel_kernel::matmul_f32_q4_1(x_data, w_bytes, m, in_features, out_features)?
            };
            ...
            *out_shape.last_mut().unwrap() = out_features;
            ...
            let result_data = if self.use_native_q8 {
                swe_llmmodel_kernel::matmul_f32_q8_v2(x_data, w_bytes, m, in_features, out_features)?
            } else {
                swe_llmmodel_kernel::matmul_f32_q8(x_data, w_bytes, m, in_features, out_features)?
            };
