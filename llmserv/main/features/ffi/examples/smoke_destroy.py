...
def count(h, text):
    n = c_size_t()
    return lib.llmserv_token_count(h, text, byref(n)), n.value
...
def main() -> None:
    ...
    lib.llmserv_destroy(handle)    # first destroy
    lib.llmserv_destroy(handle)    # second destroy — must not crash
    lib.llmserv_destroy(handle)    # third destroy — must not crash
    ...
    out = c_char_p()
    rc = lib.llmserv_complete(handle, b"hi", c_uint32(4), c_float(0.0), byref(out))
    assert rc == DESTROYED, f"expected DESTROYED (6), got {rc}"
    if out.value:
    ...
    rc = lib.llmserv_init(byref(handle2))
    assert rc == OK
    print("    init OK")
