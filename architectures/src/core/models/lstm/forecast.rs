...
mod tests {
    ...
    /// @covers: fc
    #[test]
    fn test_fc_projection_multi_output() {
    ...
    /// @covers: fc
    #[test]
    fn test_fc_accessor_returns_linear() {
        let model = LSTMForecast::new(3, 8, 1, 2);
        let fc = model.fc();
        let fc_params = fc.parameters();
        assert!(!fc_params.is_empty(), "FC layer should have parameters");
    }
    ...
    /// @covers: reset_state
    #[test]
    fn test_reset_state_restores_initial_output() {
    ...
    /// @covers: reset_state
    #[test]
    fn test_reset_state_differs_from_stateful_forward() {
        let mut model = LSTMForecast::new(3, 8, 1, 1);
        let input = Tensor::randn([1, 4, 3]);

        // First forward seeds the state
        let _out1 = model.forward(&input).unwrap();
        // Second forward WITHOUT reset carries state
        let out_stateful = model.forward(&input).unwrap();

        // Reset and re-run
        model.reset_state();
        let _out_fresh_1 = model.forward(&input).unwrap();
        model.reset_state();
        let out_fresh_1 = model.forward(&input).unwrap();

        // The stateful output (after 2 forwards) should differ from fresh (after 1 forward)
        let d_stateful = out_stateful.to_vec();
        let d_fresh = out_fresh_1.to_vec();
        let differs = d_stateful.iter().zip(d_fresh.iter()).any(|(a, b)| (a - b).abs() > 1e-7);
        assert!(differs, "Stateful forward should differ from reset forward");
    }
}
