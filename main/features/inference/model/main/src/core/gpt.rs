...
use rustml_nn::{CausalSelfAttention, LayerNorm, Linear};
...
impl GptBlock {
    ...
    pub fn from_weights(
        ...
        let get_weight = |name: &str| -> ModelResult<Tensor> {
            weights
                .get(&format!("{}.{}", prefix, name))
                .cloned()
                .ok_or_else(|| ModelError::Model(format!("Missing weight: {}.{}", prefix, name)))
        };
...
impl GptModel {
    ...
    pub fn from_weights(config: GptConfig, weights: HashMap<String, Tensor>) -> ModelResult<Self> {
        let get_weight = |name: &str| -> ModelResult<Tensor> {
            weights
                .get(name)
                .cloned()
                .ok_or_else(|| ModelError::Model(format!("Missing weight: {}", name)))
        };
    ...
    pub fn forward(&self, input_ids: &Tensor) -> ModelResult<Tensor> {
...
impl crate::api::types::LanguageModel for GptModel {
    ...
    fn forward_with_cache(&self, input_ids: &Tensor, cache: &mut rustml_nn::KVCache) -> ModelResult<Tensor> {
