...
use super::scaler_type::ScalerType;
...
mod tests {
    ...
    /// @covers: fit, transform, inverse_transform
    #[test]
    ...
    /// @covers: params
    #[test]
    fn test_params_returns_per_feature_parameters() {
        let data = vec![vec![1.0, 2.0, 3.0], vec![10.0, 20.0, 30.0]];
        let scaler = Scaler::fit(&data, ScalerType::MinMax);
        let params = scaler.params();
        assert_eq!(params.len(), 2);
        // First feature: min=1, range=2
        assert!((params[0].0 - 1.0).abs() < 1e-6);
        assert!((params[0].1 - 2.0).abs() < 1e-6);
    }
}
