...
mod tests {
    ...
    /// @covers: new
    #[test]
    fn test_create_tcn_with_valid_config() {
        let tcn = TCN::new(3, 10, 16, 3, 4);
        assert_eq!(tcn.num_channels(), 16);
        assert_eq!(tcn.output_size(), 10);
        assert_eq!(tcn.num_layers(), 4);
    }

    /// @covers: forward
    #[test]
    fn test_forward_produces_correct_output_shape() {
        let mut tcn = TCN::new(3, 10, 16, 3, 4);
        let input = Tensor::randn([2, 3, 50]); // batch=2, features=3, seq_len=50
        let output = tcn.forward(&input).unwrap();
        assert_eq!(output.shape(), &[2, 10]);
    ...
    /// @covers: receptive_field
    #[test]
    fn test_receptive_field_formula() {
        // kernel_size=3, num_layers=4: 1 + (3-1) * (2^4 - 1) = 1 + 2*15 = 31
        let tcn = TCN::new(1, 1, 8, 3, 4);
        assert_eq!(tcn.receptive_field(), 31);
    }

    /// @covers: parameters
    #[test]
    fn test_parameters_nonempty() {
        let tcn = TCN::new(3, 10, 16, 3, 2);
        let params = tcn.parameters();
        assert!(!params.is_empty(), "TCN should have parameters");
    ...
    /// @covers: num_channels
    #[test]
    fn test_num_channels_matches_config() {
        let tcn = TCN::new(3, 10, 32, 3, 2);
        assert_eq!(tcn.num_channels(), 32);
    }

    /// @covers: output_size
    #[test]
    fn test_output_size_matches_config() {
        let tcn = TCN::new(3, 10, 16, 3, 2);
        assert_eq!(tcn.output_size(), 10);
    }
