...
use swe_ml_training::{GELU, Dropout, LayerNorm, Linear};
...
mod tests {
    ...
    /// @covers: sinusoidal_positional_encoding
    #[test]
    ...
    /// @covers: train
    #[test]
    fn test_train_switches_to_training_mode() {
        let mut model = TimeSeriesTransformer::new(1, 8, 2, 1, 1, 20, 0.1);
        model.train();
        // Verify model can still forward in training mode
        let input = Tensor::randn([1, 5, 1]);
        let output = model.forward(&input).unwrap();
        assert_eq!(output.shape(), &[1, 1]);
    }
    ...
    /// @covers: eval
    #[test]
    fn test_eval_switches_to_evaluation_mode() {
        let mut model = TimeSeriesTransformer::new(1, 8, 2, 1, 1, 20, 0.1);
        model.eval();
        let input = Tensor::randn([1, 5, 1]);
        let output = model.forward(&input).unwrap();
