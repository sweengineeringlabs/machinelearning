...
fn quantize_f32_slice(data: &[f32], target: QuantTarget) -> QuantizeResult<(Vec<u8>, GGMLType)> {
    match target {
        QuantTarget::Q8_0 => {
            let quantized = swe_llmmodel_kernel::quantize_q8_0(data)?;
            Ok((quantized, GGMLType::Q8_0))
        }
        QuantTarget::Q4_0 => {
            let quantized = swe_llmmodel_kernel::quantize_q4_0(data)?;
            Ok((quantized, GGMLType::Q4_0))
        }
        QuantTarget::Q4_1 => {
            let quantized = swe_llmmodel_kernel::quantize_q4_1(data)?;
